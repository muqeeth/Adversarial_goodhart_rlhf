...
SCRATCH_CLONE_FOLDER := /network/scratch/n/noukhovm/trl_summarize
...
ifeq ($(SNAPSHOT),1)
	_WORKDIR := $(SCRATCH_CLONE_FOLDER)/$(SHORT_REVISION)
else
	_WORKDIR := $(PWD)
endif
...
# endif
# _DEEPSPEED_PREFIX := accelerate launch --config_file $(DEEPSPEED_CFG) --mixed_precision=$(FP) --num_processes $$(($(NPROC) - $(VLLM)))
#

...
def create_prompt_completions_with_prefix(batch: Dict[str, List], prefix=" 🤗🤗🤗"):
    output = {
        "prompt_chosen": [],
        "prompt_rejected": [],
    }
...
if __name__ == "__main__":
    ...
    for split in [args.train_split, args.eval_split]:
        ...
        dataset_with_prefix = dataset.map(create_prompt_completions_with_prefix, batched=True)
        scores_with_prefix = {"chosen": [], "rejected": []}
        for comp in ["chosen", "rejected"]:
            for out in tqdm(
                reward_pipeline(KeyDataset(dataset_with_prefix, f"prompt_{comp}"), batch_size=args.batch_size),
                desc=comp,
                total=len(dataset_with_prefix),
            ):
                if isinstance(out, dict):
                    out = [out]
                scores_with_prefix[comp].extend([o["score"] for o in out])

        # Debugging
        original_indices = [i for i, (c, r) in enumerate(zip(scores["chosen"], scores["rejected"])) if c > r]
        prefix_indices = [i for i, (c, r) in enumerate(zip(scores_with_prefix["chosen"], scores_with_prefix["rejected"])) if c > r]
        print(f"indices where chosen > rejected for original dataset: {original_indices}")
        print(f"indices where chosen > rejected for prefix dataset: {prefix_indices}")
        print(f"Agreement with prefix {len(set(original_indices).intersection(set(prefix_indices))) / len(set(original_indices).union(set(prefix_indices))): .2f}")
